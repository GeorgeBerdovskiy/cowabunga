    ...
    pub fn insert(&mut self, columns: &Vec<Option<i64>>) -> Result<Offset, DatabaseError> {
        ...
        for (user_column, column_value) in self.columns.iter().zip(columns.iter()) {
            offset = self
                ...
                .lock()
                .unwrap()
                .write_next_value(*user_column, *column_value)?;
        }
        ...
        self.buffer_pool_manager
            .lock()
            .unwrap()
            .write_next_value(self.columns[self.columns.len() - 1], None)?;
    ...
    pub fn get_version(&self, rid: RID, mut tail_rid: RID, relative_version: i64) -> (bool, RID) {
        ...
        while version > relative_version {
            ...
            match self.page_ranges[tail_address.range].read_tail_record(
                ...
                Ok(tail_columns) => {
                    tail_rid = tail_columns[tail_columns.len() - 1 - INDIRECTION_REV_IDX].unwrap()
                        as usize;
                }
                Err(_) => {
...
impl Table {
    ...
    pub fn merge(&mut self, page_range: &mut PageRange) {
        /// merging data
        let base_pages: Vec<LogicalPage<Base>> = page_range.base_pages;
        ...
        for page in base_pages {
            ...
            let mut copied_base: LogicalPage<Base> = LogicalPage::new(
                self.table_identifier,
                self.num_columns,
                self.buffer_pool_manager,
            );
            ...
            for offset in 0..CELLS_PER_PAGE {
                ...
                if indirection.is_none() {
                    ...
                    let tail_address = self.page_directory[&tail_rid];
                    // If tail_rid is higher than temp_tsp then change temp_tsp to tail rid
                    // and the page ranges TSP to temp_tsp
